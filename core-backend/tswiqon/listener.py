...
if __name__ == '__main__':
    ...
    try:
        ...
        rag_utils.create_and_save_faiss_index() # This line is now active
        ...
        start_listening() # This line is now active

        # logging.info("Main thread: start_listening() has exited.") # This line might be reached if start_listening stops unexpectedly.
        # main_tasks_failed = True # Treat this as a failure to keep listening

    except KeyboardInterrupt:
        logging.info("Main thread: Listener service interrupted by user.")
        # main_tasks_failed = True
    except Exception as e:
        logging.critical(f"Main thread: An unhandled exception occurred: {e}", exc_info=True)
        # main_tasks_failed = True
    # finally: # Finally block not strictly needed if start_listening loops indefinitely
        # logging.info("Main thread: Main task execution block finished or encountered an error.")

    # If start_listening() is truly blocking and only exits on error/interrupt,
    # code here might indicate an issue or normal shutdown path after interrupt.
    logging.info("Main thread: Listener service shutting down or start_listening() exited.")